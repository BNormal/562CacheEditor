...
public final class InputStream extends Stream {
	...
	public void initBitAccess() {
		setBitPosition(offset * 8);
	}
	
	private final static int[] BIT_MASK = new int[] { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023,
	...
	public void finishBitAccess() {
		offset = (7 + getBitPosition()) / 8;
	}

	public int readBits(int bitOffset) {

		int bytePos = getBitPosition() >> 1779819011;
		int i_8_ = -(0x7 & getBitPosition()) + 8;
		setBitPosition(getBitPosition() + bitOffset);
		int value = 0;
		for (/**/; bitOffset < i_8_; i_8_ = 8) {
			value += (getBitMask()[i_8_] & getBuffer()[bytePos++]) << -i_8_ + bitOffset;
			bitOffset -= i_8_;
		}
		if (i_8_ == bitOffset)
			value += getBuffer()[bytePos] & getBitMask()[i_8_];
		else
			value += (getBuffer()[bytePos] >> -bitOffset + i_8_ & getBitMask()[bitOffset]);
		return value;
	}
	...
	public void checkCapacity(int length) {
		if (offset + length >= getBuffer().length) {
	...
	public int readByte() {
		return getRemaining() > 0 ? getBuffer()[offset++] : 0;
	}
