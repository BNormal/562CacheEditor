		...
		synchronized(data) {
			try {
				...
				while (archiveLength > readBytesCount) {
					...
					int dataToReadCount = archiveLength - readBytesCount;
					if (dataToReadCount > 512)
						dataToReadCount = 512;
					data.read(readCachedBuffer, 0, 8 + dataToReadCount);
					int currentArchive = (0xff & readCachedBuffer[1]) + (0xff00 & readCachedBuffer[0] << 8);
					int currentPart = ((readCachedBuffer[2] & 0xff) << 8) + (0xff & readCachedBuffer[3]);
					int nextSector = (readCachedBuffer[6] & 0xff) + (0xff00 & readCachedBuffer[5] << 8) + ((0xff & readCachedBuffer[4]) << 16);
					int currentIndex = readCachedBuffer[7] & 0xff;
					...
					for (int index = 0; dataToReadCount > index; index++)
						archive[readBytesCount++] = readCachedBuffer[8 + index];
					part++;
					sector = nextSector;
				}
				return archive;
