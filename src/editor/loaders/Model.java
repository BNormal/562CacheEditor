...
import editor.util.rsmv.Surface;
...
public class Model implements Cloneable {
	...
	public byte[] encode(Model model) {
		...
		if (numVertices > 0) {
			for (int i = 0; i < numVertices; i++) {
				...
				if (anIntArray1500 != null)
					stream.writeByte(anIntArray1500[i]);
			}
		}
		for (int i = 0; i < numTriangles; i++) {
			stream.writeShort(faceColors[i]);
			if (faceRenderType != null)
				stream.writeByte(faceRenderType[i]);
			if (trianglePriorities != null)
				stream.writeByte(trianglePriorities[i]);
			if (faceAlpha != null)
				stream.writeByte(faceAlpha[i]);
			if (triangleSkinValues != null)
				stream.writeByte(triangleSkinValues[i]);
			if (faceTexture != null)
				stream.writeShort(faceTexture[i]);
			if (hasTexture != null)
				if (faceTexture[i] != -1)
					stream.writeByte(hasTexture[i]);
		}
	...
	public void newHeader(byte[] arg0, int arg1) {
	    ...
	    if (has_large_size || has_vertex_normals)
	      isNewModel = true;
	    //
		...
		if (has_fill_attr)
			faceRenderType = new byte[numTriangles];//faceRenderType
		if (model_priority == 255)
			trianglePriorities = new byte[numTriangles];
		else
			priority = (byte) model_priority;
		if (has_skins == 1)
			anIntArray1500 = new int[numVertices];
		...
		if (has_triangle_skin_types == 1)
			this.triangleSkinValues = new short[numTriangles];
		...
		if (has_textures == 1 && numTexTriangles > 0)
		...
		for (int i_119_ = 0; i_119_ < numVertices; i_119_++) {
			...
			int i_121_ = 0;
			if ((temp_i_120_[i_119_] & 0x1) != 0)
				i_121_ = stream_72_.readUnsignedSmart3();
			int i_122_ = 0;
			if ((0x2 & temp_i_120_[i_119_]) != 0)
				i_122_ = stream_73_.readUnsignedSmart3();
			int i_123_ = 0;
			if ((0x4 & temp_i_120_[i_119_]) != 0)
				i_123_ = stream_74_.readUnsignedSmart3();
			...
			if (has_skins == 1)
				anIntArray1500[i_119_] = stream_75_.readUnsignedByte();
		}
		...
		for (int tri_x = 0; (numTriangles ^ 0xffffffff) < (tri_x ^ 0xffffffff); tri_x++) {
			...
			if (has_fill_attr)
				this.faceRenderType[tri_x] = (byte) stream_72_.readByte();//faceRenderType
			if (model_priority == 255)
				trianglePriorities[tri_x] = (byte) stream_73_.readByte();//trianglePriorities
			if (has_alpha == 1)
				faceAlpha[tri_x] = (byte) stream_74_.readByte();//faceAlpha
			if (has_triangle_skin_types == 1)
				this.triangleSkinValues[tri_x] = (short) stream_75_.readUnsignedByte();//triangleSkinValues
			if (has_textures == 1)
				faceTexture[tri_x] = (short) (-1 + stream_76_.readUnsignedShort());//faceTextures
			if (hasTexture != null) {//textureCoords
				if (faceTexture[tri_x] == -1)
					hasTexture[tri_x] = (byte) -1;
				else
					hasTexture[tri_x] = (byte) (stream_77_.readUnsignedByte() + -1);
			}
		}
		...
		for (int i_129_ = 0; i_129_ < numTriangles; i_129_++) {
			...
			if (i_130_ == 1) {
				...
				if ((i_125_ ^ 0xffffffff) < (maxDepth ^ 0xffffffff))
					maxDepth = i_125_;
				if (i_126_ > maxDepth)
					maxDepth = i_126_;
				if (i_127_ > maxDepth)
			...
			if (i_130_ == 2) {
				...
				triangleViewspaceY[i_129_] = i_126_;
				triangleViewspaceZ[i_129_] = i_127_;
				if ((i_127_ ^ 0xffffffff) < (maxDepth ^ 0xffffffff))
					maxDepth = i_127_;
			}
			if (i_130_ == 3) {
				i_125_ = i_127_;
		...
		if (has_surface_fx) {
			...
			if (temp_isolated_normal_count > 0) {
				...
				for (int face = 0; face < temp_isolated_normal_count; face++) {
					...
					byte pri;
					if (model_priority == 255)
						pri = trianglePriorities[temp_face_point[face]];
					else
						pri = (byte) model_priority;
	...
	public void oldHeader(boolean arg0, byte[] arg1) {
		...
		for (int i_45_ = 0; i_45_ < numVertices; i_45_++) {
			...
			if ((i_46_ & 0x1 ^ 0xffffffff) != -1)
				i_47_ = stream_16_.readUnsignedSmart3();
			int i_48_ = 0;
			if ((i_46_ & 0x2) != 0)
				i_48_ = stream_17_.readUnsignedSmart3();
			int i_49_ = 0;
			if ((0x4 & i_46_ ^ 0xffffffff) != -1)
				i_49_ = stream_18_.readUnsignedSmart3();
			...
			if ((i_23_ ^ 0xffffffff) == -2)
				anIntArray1500[i_45_] = stream_19_.readUnsignedByte();
		}
		stream.offset = i_37_;
		...
		for (int i_50_ = 0; (numTriangles ^ 0xffffffff) < (i_50_ ^ 0xffffffff); i_50_++) {
			...
			if ((i_20_ ^ 0xffffffff) == -256)
				trianglePriorities[i_50_] = (byte) stream_17_.readByte();
			if (i_21_ == 1)
				faceAlpha[i_50_] = (byte) stream_18_.readByte();
			if ((i_22_ ^ 0xffffffff) == -2)
		...
		for (int i_56_ = 0; i_56_ < numTriangles; i_56_++) {
			...
			if ((i_57_ ^ 0xffffffff) == -3) {
				...
				triangleViewspaceY[i_56_] = i_53_;
				triangleViewspaceZ[i_56_] = i_54_;
				if ((maxDepth ^ 0xffffffff) > (i_54_ ^ 0xffffffff))
					maxDepth = i_54_;
			}
			if (i_57_ == 3) {
				i_52_ = i_54_;
