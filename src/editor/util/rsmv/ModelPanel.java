...
public class ModelPanel extends JPanel implements Runnable {
	...
	public ModelPanel() {
		...
		setLayout(layout);
		layout.setHorizontalGroup(layout.createParallelGroup(Alignment.LEADING).addGap(0, 536, 32767));
		layout.setVerticalGroup(layout.createParallelGroup(Alignment.LEADING).addGap(0, 318, 32767));*/
	}
	...
	public void run() {
		Graphics g = null;
		while (true) {
			if (CacheEditor.getSelectedTab() == 5) {
	...
	public void loadModelFromBytes(byte[] bytes) {
		...
		if (bytes != null && bytes[bytes.length - 1] == -1 && bytes[bytes.length - 2] == -1) {
			if ((model = ModelRSMV.loadModel(bytes)).isNewHeader) {
				model.downScaleModel(4);
			}
		} else {
			ModelRSMV.initModel(bytes);
